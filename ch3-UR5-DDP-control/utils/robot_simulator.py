...
class RobotSimulator:
    ...
    # Class constructor
    def __init__(self, conf, robot: RobotWrapper):
        self.conf = conf
        self.robot = robot # RobotWrapper
        ...
        self.na = na = robot.na         # number of actuated joints = nq
        # Matrix S used as filter of vetor of inputs U
        self.S = np.hstack((np.zeros((na, nv-na)), np.eye(na, na))) # [0, I]
        ...
        # setup gepetto viewer, RobotWrapper.viz
        ...
        if(conf.use_viewer):
            ...
            # 2) Connect the CORBA client
            try:
                gepetto.corbaserver.Client()
            except Exception as e:
                print(f"[Warning] could not start CORBA client: {e}")

            # 3) Create & initialize the visualizer
            # self.robot.viz is the gepetto viewer class
            self.robot.initViewer(loadModel=True)
            print(self.robot.viz)
            
            if(conf.show_floor):
                self.robot.viz.viewer.gui.createSceneWithFloor('world')
                self.robot.viz.viewer.gui.setLightingMode('world/floor', 'OFF')
            try:  
                self.robot.viz.viewer.gui.setCameraTransform("python-pinocchio", conf.CAMERA_TRANSFORM)
            except:
                self.robot.viz.viewer.gui.setCameraTransform(0, conf.CAMERA_TRANSFORM)
    ...
    def step(self, u, dt=None):
        ...
        if(self.simulate_coulomb_friction and self.simulation_type=='timestepping'):
            ...
            # S is the matrix that filters the input u to the actuated joints
            # solve x that Mx = b
...
if __name__=='__main__':
    ...
    from robot_loaders import loadUR
